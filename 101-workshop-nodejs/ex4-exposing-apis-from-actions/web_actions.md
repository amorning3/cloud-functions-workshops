...
Web actions are actions which can be called externally using the HTTP protocol from clients like `curl` or web browsers. IBM Cloud Functions (ICF) provides a simple flag, `--web true`, which causes it to automatically create an HTTP accessible URL (endpoint) for any action.
...
3. Invoke the web action URL returned using the `curl` command:
      ...
      This is because we need to tell ICF what content type we expect the function to return.
...
2. Retrieve the URL for new web action:
...
{% hint style="success" %}
Aren’t web actions are cool? They are a great feature that allows you to make you’re actions accessible on the web while controlling content types.  You can even generate HTML and other website content on the fly without hosting a content server!
{% endhint %}
